...
None of the magenta boxes or their text may appear in the photograph; they are direction for you, not decoration.`
      : "";
  const mapBit = [
    options?.tintedPlan && options?.geometryLock
      ? "Attachment 1 is THIS APARTMENT'S OWN PLAN — the architect's drawing, cropped to the flat, with a translucent wash over each room in a colour that says what the room is for and its name printed beside it. Attachment 2 is the SAME flat measured off that drawing and built as a 3D massing: same walls, same rooms, same positions, in blocks. They agree, and between them they are the whole layout: the ink gives you every wall, jog, door swing, window, fixture and furniture symbol exactly where it is drawn; the massing gives you the depth and the size of each space. Repaint that flat as the photoreal still with nothing moved — a room that appears in a different place than in these two attachments is a failed still. Neither the wash, the printed names nor the massing's flat colours appear in your photograph."
      : options?.tintedPlan
