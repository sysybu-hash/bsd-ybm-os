...
- Windows exist ONLY where the plan breaks the wall hatch for one. A stretch of outer wall drawn as unbroken hatch is a solid wall — never cut a window, a glazed slot or a light strip into it because a room would otherwise look dark. Walk each outer wall on the sheet, count the openings drawn in it, and render exactly those, in those positions.
- ONE seating group in the apartment, in the room where the plan draws sofas or armchairs. The entrance is circulation: bare floor, at most a console, a mirror or a coat hook. No sofa, no armchair, no rug and no coffee table at the entrance, and no second lounge anywhere the plan does not draw one.
