...
- entranceDoorMissing: true ONLY if you can see, in the still, the stretch of outer wall where the PLAN draws the front door (its swing, leaf or entrance triangle, usually next to חדר מדרגות / מעלית / the corridor), and that stretch is unbroken solid wall — no leaf, no frame, no doorway. A sealed apartment you cannot enter is this failure, and a balcony sliding door is NOT the front door. Answer false if a door leaf, a frame or a clear doorway is there, and also false if that piece of wall is cut away by the bird's-eye view, hidden behind furniture, out of frame, or you cannot tell: this answer blocks the still, so say true only for a wall you have actually looked at and found closed.
