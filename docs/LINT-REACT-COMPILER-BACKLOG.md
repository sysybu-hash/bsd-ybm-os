...
## 2026-08-26 — `refs` הודלק. **כל ארבעת החוקים אכיפים.**

`eslint.config.js` כבר לא מכבה שום חוק.

### 43 מתוך 55 היו תוצאה חיובית שגויה אחת

`CrmTableWidget` החזיק את כל התוצאה של `useCrmTable` באובייקט `s` יחיד. מכיוון
שהאובייקט נושא `fileInputRef`, ה-Compiler הסיק שכל `s.loading` וכל `s.clients`
הם **גישה ל-ref בזמן רנדר**.

פירוק ה-destructuring לשמות בודדים ניקה את כל 43 בבת אחת, בלי שינוי התנהגות.

**הלקח:** hook שמחזיר ref מעורבב עם state באובייקט אחד "מזהם" את כל האובייקט
מבחינת ה-Compiler. **אל תחזירו ref בתוך אובייקט תוצאה גדול** — או שהצרכן חייב
לפרק לשמות.
...
| כמה | מה | הטיפול |
|---|---|---|
| 7 | דפוס latest-ref (`onFooRef.current = onFoo` בגוף הרנדר) | `hooks/use-latest-ref.ts` — השמה ב-`useInsertionEffect` |
| 1 | **באג אמיתי** | `SettingsTab` חישב `isDirty` מ-ref. הדגל שולט על באנר "שינויים לא שמורים", ו-ref שנכתב מאפקט אינו גורם לרנדר — כך שהבאנר יכול היה לשרוד את השמירה שהוא מתאר |
| 4 | ה-adaptive shell | `eslint-disable` מתועד |

מתוך הארבעה: שניים הם אתחולי `useState` עצלים שרצים פעם אחת ב-mount, וה-חוק
אינו יכול להבחין ביניהם לבין גוף הרנדר. השניים האחרים **באמת** קוראים גבולות
מדודים בזמן רנדר; התיקון הכן הוא שיקוף הגבולות ל-state מאחורי `ResizeObserver`,
שעולה רנדר לכל פריים של resize ונוגע במנהל החלונות ש-`e2e/window-scroll.spec.ts`
שומר עליו.

### מה ש-`useLatestRef` עלה — שווה לדעת

`useRef` inline **מזוהה** ע"י ה-Compiler כ-ref ומוחרג ממערכי תלויות.
`useLatestRef` מחזיר ערך אטום, ולכן ה-refs חייבים להופיע בתלויות. הם יציבים
לאורך חיי הקומפוננטה, אז זה חינם בזמן ריצה.

בנוסף, זה **הפסיק את ה-bailout** של ה-Compiler בשני hooks — מה שחשף שני מקרי
`set-state-in-effect` קיימים (תמיכת דיבור בדפדפן, ו-client id חסר של PayPal),
שניהם ניתנים לגזירה בזמן רנדר ונגזרים שם עכשיו.
