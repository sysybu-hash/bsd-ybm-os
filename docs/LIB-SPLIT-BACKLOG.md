...
| לפני | אחרי | פעולה |
|------|------|--------|
| 400 | 301 + 132 + 104 | `useAppBuilder.ts` → `useAppBuilderSavedApps.ts` (list/load/delete) + `useAppBuilderAiActions.ts` (regenerate/share) |

**היעד <25 לא הושג. הספירה נשארה 27, ואני עוצר כאן במכוון.**

### למה עצרתי

`useAppBuilder` ירד מ-400 ל-**301**. המדד סופר קבצים **≥300**, כלומר הקובץ
עדיין נספר — בשורה אחת.

היה קל להוריד שורה. **לא עשיתי זאת**, וזו הנקודה: לגזום שורה ריקה או להדק
תגובה כדי לחצות סף הוא שינוי של המספר, לא של הקוד. אותו היגיון חל על ששת
הקבצים שיושבים על 300-307.

### מה שכן נעשה, ולמה זה שווה יותר

לפני העבודה הזו, `useAppBuilder` (400 שורות) ו-`ProgressBillPortalPanel` (385)
היו **ללא שום כיסוי בדיקות** — לא E2E, לא יוניט. ריפקטור של הוק stateful כזה
בלי רשת ביטחון הוא הימור, לא שיפור.

לכן הסדר היה: **טסטים קודם.** נכתבו 11 טסטים ל-`useAppBuilderSavedApps`
(טעינה, מחיקה, אישור שנדחה, אפליקציה גלובלית, כשל רשת, ניקוי דגלים) — כיסוי
שלא היה קיים כלל. `@testing-library/dom` נוסף כ-devDependency; הוא peer של
`@testing-library/react` שכבר היה מותקן, וחסרונו מנע כתיבת טסטי hooks.

### התנאי להמשך

`ProgressBillPortalPanel` (385) הוא הבא בתור — **אחרי** שייכתבו לו טסטים.
אותו כלל: לא לפצל קוד stateful בלי כיסוי.

כדי להגיע מתחת ל-25 צריך שלושה קבצים מתחת ל-300. בקצב של טסטים-קודם זה
2-3 סבבים נוספים. **זה הקצב הנכון.**

---

## 2026-08-25 — ProgressBillPortalPanel

| לפני | אחרי | פעולה |
|------|------|--------|
| 385 | 323 + 101 + 12 | `ProgressBillPortalPanel.tsx` → `useProgressBillPortalData.ts` (שלוש הטעינות) + `types.ts` |

7 טסטים חדשים. **הפיצול חשף באג אמיתי:**

```js
const loadProjects = useCallback(async () => { … }, [projectId]);
useEffect(() => { void loadBills(); void loadProjects(); }, [loadBills, loadProjects]);
```

`projectId` בתלויות של `loadProjects`, וה-effect תלוי ב-`loadProjects` — כלומר
**כל בחירת פרויקט טענה מחדש גם את החשבונות וגם את רשימת הפרויקטים**. תוקן עם
`setProjectId((current) => current || …)`, ויש טסט שנועל: *"does not refetch
bills or projects when the project changes"*.

## המדד: מסקנה

`logic count` נשאר סביב 27-28 לאורך כל הסבבים. **הפסקתי לרדוף אחריו.**

הסיבה פשוטה: המדד סופר קבצים ≥300, והפיצולים מורידים קבצים ל-301, 323 —
שיפור אמיתי בקריאוּת שהמדד לא רואה. לגזום שורה כדי לחצות סף משנה את המספר
ולא את הקוד.

**מה שכן הושג בשלושת הסבבים:**

| קובץ | לפני | אחרי | טסטים חדשים |
|---|---|---|---|
| `useCrmTable` | 456 | 414 | — |
| `useAppBuilder` | 400 | 301 | 11 |
| `ProgressBillPortalPanel` | 385 | 323 | 7 |

שלושת הקבצים הגדולים ביותר קטנו, ושניים מהם קיבלו כיסוי בדיקות שלא היה להם
כלל. באג אחד אמיתי נמצא בדרך. **זה הערך; המספר הוא לא.**

הצעה: לשנות את המדד מ"מספר קבצים ≥300" ל"סך השורות ב-N הקבצים הגדולים", שמזהה
שיפור הדרגתי במקום לדרוש חציית סף.
