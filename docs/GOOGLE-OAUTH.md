...
| **Scopes רגישים** | למשל `contacts.readonly` (Sensitive). `drive.file` הוא non-sensitive; אם רשום באותו Client עם scopes רגישים — עדיין עשויה להופיע אזהרה עד Publish. |
...
| **Google Contacts (CRM)** | `/api/integrations/google/contacts/connect` | + `https://www.googleapis.com/auth/contacts.readonly` |

קבצי מפתח: `lib/auth.ts`, `lib/google-account-tokens.ts`, `lib/google-contacts-oauth.ts`.
...
כדי ש**מסך הכניסה** לא יציג scopes של Drive/Contacts:
...
- **שם:** `BSD-YBM Sign-In`
- **Scopes ב-Data access:** רק `openid`, `email`, `profile` (ללא Drive/Contacts)
- **Redirect URIs:**
...
- **Scopes:** `openid`, `email`, `profile`, `drive`, `contacts.readonly` (לפי הצורך)
- **Redirect URIs:**
...
`contacts.readonly` — **Sensitive**; ייתכן טופס נפרד או אותו תהליך.
