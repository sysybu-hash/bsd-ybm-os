"use client";

/**
 * Shared error-boundary UI for App Router route segments.
 * Each segment's `error.tsx` renders this with its own `route` tag so Sentry
 * reports stay attributable per segment (CLAUDE.md §2, §10).
 */
import { useEffect } from "react";
import Link from "next/link";
import * as Sentry from "@sentry/nextjs";
import { useI18n } from "@/components/os/system/I18nProvider";

type Props = Readonly<{
  error: Error & { digest?: string };
  reset: () => void;
  /** Route identifier reported to Sentry, e.g. "app/home". */
  route: string;
  /** Where the secondary action links to. Defaults to the workspace. */
  homeHref?: string;
}>;

export default function RouteErrorBoundary({ error, reset, route, homeHref = "/app" }: Props) {
  const { t, dir } = useI18n();

  useEffect(() => {
    try {
      Sentry.captureException(error, { extra: { digest: error.digest, route } });
    } catch {
      /* Sentry not configured */
    }
  }, [error, route]);

  const isDev = process.env.NODE_ENV === "development";

  return (
    <div
      dir={dir}
      className="flex min-h-screen flex-col items-center justify-center bg-[color:var(--background-main)] px-6 py-16 text-[color:var(--foreground-main)]"
    >
      <div className="flex w-full max-w-md flex-col gap-6 text-center">
        <div className="flex justify-center">
          <div className="flex h-16 w-16 items-center justify-center rounded-2xl border border-red-500/20 bg-red-500/10 text-red-400">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              className="h-8 w-8"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
              strokeWidth={1.5}
              aria-hidden="true"
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126ZM12 15.75h.008v.008H12v-.008Z"
              />
            </svg>
          </div>
        </div>

        <div className="flex flex-col gap-2">
          <h1 className="text-xl font-bold">{t("siteErrors.workspaceErrorTitle")}</h1>
          <p className="text-sm leading-relaxed text-[color:var(--foreground-muted)]">
            {t("siteErrors.workspaceErrorBody")}
          </p>
          {isDev && error?.message ? (
            <pre className="mt-2 overflow-auto rounded-xl border border-[color:var(--border-main)] bg-[color:var(--surface-card)] p-3 text-start text-xs text-red-300">
              {error.message}
            </pre>
          ) : null}
        </div>

        <div className="flex flex-col gap-3 sm:flex-row sm:justify-center">
          <button
            type="button"
            onClick={() => reset()}
            className="rounded-xl bg-indigo-600 px-6 py-2.5 text-sm font-bold text-white transition hover:bg-indigo-500 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-400"
          >
            {t("siteErrors.retry")}
          </button>
          <Link
            href={homeHref}
            className="rounded-xl border border-[color:var(--border-main)] bg-[color:var(--surface-card)] px-6 py-2.5 text-sm font-bold text-[color:var(--foreground-main)] transition hover:bg-white/[0.07]"
          >
            {t("siteErrors.backToDashboard")}
          </Link>
        </div>
      </div>
    </div>
  );
}
