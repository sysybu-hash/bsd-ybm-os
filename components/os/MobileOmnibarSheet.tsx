      ...
      {open ? (
        <>
          ...
          <motion.div
            role="dialog"
            aria-modal="true"
            aria-label="שורת פקודה וחיפוש"
            data-testid="mobile-omnibar-sheet"
            initial={{ y: "100%" }}
            animate={{ y: 0 }}
            exit={{ y: "100%" }}
            transition={{ type: "spring", damping: 28, stiffness: 320 }}
            className="fixed inset-x-0 bottom-0 z-[60] max-h-[85dvh] overflow-y-auto rounded-t-2xl border border-[color:var(--border-main)] border-b-0 bg-[color:var(--background-main)]/98 p-4 pb-[max(1rem,env(safe-area-inset-bottom))] shadow-2xl backdrop-blur-xl"
          >
            ...
            <div className="mb-3 flex items-center justify-between gap-2">
              <span className="text-xs font-black tracking-wide text-[color:var(--foreground-main)]">פקודה וחיפוש</span>
              <button
                type="button"
                onClick={onClose}
