  ...
  return (
    <section>
      <div className="mb-4 flex items-center gap-2">
        ...
        <h3 className="text-sm font-bold text-[color:var(--foreground-muted)]">{t("workspaceWidgets.invoice.client")}</h3>
      </div>

      <div ref={wrapRef} className="relative space-y-3">
        <div className="relative flex gap-1">
          <div className="relative min-w-0 flex-1">
            <User className="pointer-events-none absolute end-3 top-3 h-4 w-4 text-[color:var(--foreground-muted)]" />
            <input
              ref={inputRef}
              type="text"
              autoComplete="off"
              disabled={loading}
              value={name}
              placeholder={t(loading ? "workspaceWidgets.invoice.loadingClients" : "workspaceWidgets.invoice.clientPlaceholder")}
              ...
              className="w-full rounded-xl border border-[color:var(--border-main)] bg-[color:var(--surface-card)]/50 py-3 ps-10 pe-4 text-sm text-[color:var(--foreground-main)] focus:outline-none focus:ring-1 focus:ring-emerald-500/50"
            />
          </div>
          <OsIconButton
            label={t("workspaceWidgets.invoice.openClientList")}
            className="h-[46px] w-11"
        ...
        {open && suggestions.length > 0 ? (
              ...
              <li key={c.id}>
                ...
                >
                  ...
                  <span className="text-[10px] text-[color:var(--foreground-muted)]">
                    {c.email || c.phone || t("workspaceWidgets.invoice.noContactInfo")}
                  </span>
                </button>
              </li>
            ))}
          ...
          <p className="absolute z-[100] w-full rounded-xl border border-[color:var(--border-main)] bg-[color:var(--surface-card)] px-3 py-2 text-xs text-[color:var(--foreground-muted)] shadow-lg">
            {t("workspaceWidgets.invoice.noCrmClients")}
          </p>
        ) : null}

        {selected && !isNewClient ? (
          <p className="text-xs text-emerald-600 dark:text-emerald-400">
            {t("workspaceWidgets.invoice.existingClient", { name: selected.name })}
        ...
        {isNewClient && name.trim().length >= 2 ? (
          <div className="space-y-3 rounded-xl border border-amber-200/60 bg-amber-50/50 p-4 dark:border-amber-900/40 dark:bg-amber-950/20">
            <p className="text-xs font-bold text-amber-900 dark:text-amber-100">
              {t("workspaceWidgets.invoice.newClientHint")}
            </p>
            <div className="relative">
              <Mail className="absolute end-3 top-3 h-4 w-4 text-[color:var(--foreground-muted)]" />
              <input
                type="email"
                inputMode="email"
                value={newClient.email}
                onChange={(e) => onNewClientChange({ email: e.target.value })}
                placeholder={t("workspaceWidgets.invoice.emailOptional")}
                className="w-full rounded-lg border border-[color:var(--border-main)] bg-[color:var(--surface-card)]/80 py-2.5 ps-10 pe-3 text-sm focus:outline-none focus:ring-1 focus:ring-amber-500/40"
              />
            </div>
            <div className="relative">
              <Phone className="absolute end-3 top-3 h-4 w-4 text-[color:var(--foreground-muted)]" />
              <input
                type="tel"
                inputMode="tel"
                value={newClient.phone}
                onChange={(e) => onNewClientChange({ phone: e.target.value })}
                placeholder={t("workspaceWidgets.invoice.phoneOptional")}
                className="w-full rounded-lg border border-[color:var(--border-main)] bg-[color:var(--surface-card)]/80 py-2.5 ps-10 pe-3 text-sm focus:outline-none focus:ring-1 focus:ring-amber-500/40"
              />
            </div>
            <div className="relative">
              <FileText className="absolute end-3 top-3 h-4 w-4 text-[color:var(--foreground-muted)]" />
              <input
                type="text"
                value={newClient.notes}
                onChange={(e) => onNewClientChange({ notes: e.target.value })}
                placeholder={t("workspaceWidgets.invoice.notesOptional")}
                className="w-full rounded-lg border border-[color:var(--border-main)] bg-[color:var(--surface-card)]/80 py-2.5 ps-10 pe-3 text-sm focus:outline-none focus:ring-1 focus:ring-amber-500/40"
              />
            </div>
          </div>
