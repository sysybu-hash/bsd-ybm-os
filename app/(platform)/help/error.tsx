...
export default function HelpError({ error, reset }: Props) {
  useEffect(() => {
    Sentry.captureException(error, { extra: { digest: error.digest, route: "help" } });
  }, [error]);
  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-[color:var(--background-main)] px-4 py-12 text-[color:var(--foreground-main)]">
      <div className="flex w-full max-w-sm flex-col gap-6 text-center">
        ...
        <h1 className="text-xl font-bold">שגיאה בטעינת עמוד העזרה</h1>
        <p className="text-sm text-[color:var(--foreground-muted)]">לא ניתן לטעון את תוכן העזרה. נסה שוב.</p>
