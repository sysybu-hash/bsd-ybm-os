  ...
  return (
    ...
    >
      ...
      <main className="mx-auto flex w-full max-w-3xl flex-1 flex-col gap-8 px-6 py-12">
        {/* Title */}
        <div className="h-9 w-1/2 animate-pulse rounded-xl bg-[color:var(--border-main)]" />

        {/* Search bar */}
        <div className="h-11 w-full animate-pulse rounded-xl bg-[color:var(--border-main)]" />

        {/* FAQ sections */}
        {Array.from({ length: 4 }).map((_, section) => (
          <div key={section} className="flex flex-col gap-3" style={{ animationDelay: `${section * 80}ms` }}>
            <div className="h-5 w-1/3 animate-pulse rounded-lg bg-[color:var(--border-main)]" />
            {Array.from({ length: 3 }).map((_, item) => (
              <div
                key={item}
                className="h-14 w-full animate-pulse rounded-xl border border-[color:var(--border-main)] bg-[color:var(--border-main)]"
