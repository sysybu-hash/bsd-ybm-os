...
export default function RegisterError({ error, reset }: Props) {
  useEffect(() => {
    Sentry.captureException(error, { extra: { digest: error.digest, route: "register" } });
  }, [error]);
  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-[color:var(--background-main)] px-4 py-12 text-[color:var(--foreground-main)]">
      <div className="flex w-full max-w-sm flex-col gap-6 text-center">
        ...
        <h1 className="text-xl font-bold">שגיאה בטעינת דף ההרשמה</h1>
        <p className="text-sm text-[color:var(--foreground-muted)]">אירעה שגיאה בלתי צפויה. אנא נסה שוב.</p>
