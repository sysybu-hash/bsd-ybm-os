...
export default function SettingsError({ error, reset }: Props) {
  useEffect(() => {
    Sentry.captureException(error, {
      extra: { digest: error.digest, route: "app/settings" },
    });
  }, [error]);
    ...
    >
      <div className="flex w-full max-w-sm flex-col gap-5 text-center">
        ...
        <div className="flex flex-col gap-1">
          <h2 className="text-lg font-bold">שגיאה בטעינת ההגדרות</h2>
          <p className="text-sm text-white/50">לא הצלחנו לטעון את הגדרות החשבון.</p>
          {isDev && error?.message && (
